...
def plan_itinerary(
    ...
    # keep only POIs with coordinates
    remaining = []
    for p in pois:
        ...
        for _ in range(slots_per_day):
            ...
            for i, poi in enumerate(remaining):
                ...
                if current is not None:
                    clat, clon = current.get("lat"), current.get("lon")
                    plat, plon = poi.get("lat"), poi.get("lon")
                    if None not in (clat, clon, plat, plon):
                        travel_km = haversine_km(clat, clon, plat, plon)
            ...
            if current is not None:
                clat, clon = current.get("lat"), current.get("lon")
                plat, plon = chosen.get("lat"), chosen.get("lon")
                if None not in (clat, clon, plat, plon):
                    travel_km = haversine_km(clat, clon, plat, plon)
                    travel_mins = travel_minutes_km(travel_km, travel_mode)

            chosen["_travel_from_prev_mins"] = int(travel_mins)
            chosen["_travel_from_prev_km"] = round(float(travel_km), 2)
